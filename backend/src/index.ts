import { distBase64 } from './dist-data';
...
export default {
  async fetch(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
    ...
    if (url.pathname === '/download-dist') {
      const binaryString = atob(distBase64);
    ...
    async function syncUser(request: Request) {
      if (!userId) return;
      try {
    ...
    function safeParseGenres(genres: any) {
      if (!genres) return [];
    ...
    if (url.pathname.match(/^\/api\/chapters\/\d+$/) && method === 'GET') {
      const id = url.pathname.split('/').pop();
      const chapter: any = await env.DB.prepare('SELECT * FROM chapters WHERE id = ?').bind(id).first();
      ...
      if (chapter.is_locked === 1) {
        ...
        if (!unlocked) {
          const story: any = await env.DB.prepare('SELECT author_id FROM stories WHERE id = ?').bind(chapter.story_id).first();
          const user: any = await env.DB.prepare('SELECT id FROM users WHERE encrypted_yw_id = ?').bind(userId).first();
    ...
    if (url.pathname === '/api/chapters/unlock' && method === 'POST') {
      ...
      const { chapter_id } = await request.json() as any;
      const chapter: any = await env.DB.prepare('SELECT * FROM chapters WHERE id = ?').bind(chapter_id).first();
      const user: any = await env.DB.prepare('SELECT * FROM users WHERE encrypted_yw_id = ?').bind(userId).first();
      const story: any = await env.DB.prepare('SELECT author_id FROM stories WHERE id = ?').bind(chapter.story_id).first();
    ...
    if (url.pathname === '/api/favorites' && method === 'POST') {
      if (!isLogin) return new Response(JSON.stringify({ error: 'Unauthorized' }), { status: 401, headers });
      await syncUser(request);
      const { story_id } = await request.json() as any;
      try {
    ...
    if (url.pathname === '/api/payment/withdraw' && method === 'POST') {
      if (!isLogin) return new Response(JSON.stringify({ error: 'Unauthorized' }), { status: 401, headers });
      await syncUser(request);
      const { amount, wallet_number } = await request.json() as any;
      const user: any = await env.DB.prepare('SELECT * FROM users WHERE encrypted_yw_id = ?').bind(userId).first();
    ...
    if (url.pathname.match(/^\/api\/admin\/deposits\/\d+\/approve$/) && method === 'POST') {
      ...
      const tx: any = await env.DB.prepare('SELECT * FROM transactions WHERE id = ?').bind(id).first();
    ...
    if (url.pathname === '/api/my-stories' && method === 'POST') {
      if (!isLogin) return new Response(JSON.stringify({ error: 'Unauthorized' }), { status: 401, headers });
      const { title, description, genres, cover } = await request.json() as any;
    ...
    if (url.pathname.match(/^\/api\/my-stories\/\d+\/chapters$/) && method === 'POST') {
      ...
      const { title, content, price, is_locked } = await request.json() as any;
      
      const story: any = await env.DB.prepare('SELECT author_id FROM stories WHERE id = ?').bind(storyId).first();
      const user: any = await env.DB.prepare('SELECT id FROM users WHERE encrypted_yw_id = ?').bind(userId).first();
      if (!story || story.author_id !== user.id) return new Response(JSON.stringify({ error: 'Forbidden' }), { status: 403, headers });

      const maxOrder: any = await env.DB.prepare('SELECT MAX(chapter_order) as max_order FROM chapters WHERE story_id = ?').bind(storyId).first();
    ...
    // Admin: Get Requests (Missing in previous file)
    ...
    // Author Request (Missing in previous file)
    if (url.pathname === '/api/author-request' && method === 'POST') {
      if (!isLogin) return new Response(JSON.stringify({ error: 'Unauthorized' }), { status: 401, headers });
      await syncUser(request);
      const { reason } = await request.json() as any;
      await env.DB.prepare('INSERT INTO author_requests (user_id, reason) VALUES ((SELECT id FROM users WHERE encrypted_yw_id = ?), ?)').bind(userId, reason).run();
      return new Response(JSON.stringify({ success: true }), { headers });
    }
